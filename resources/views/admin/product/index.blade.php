@extends('layout.admin', ['title'=> 'Все товары'])
...
@section('content')
    ...
    <ul>
        @foreach($roots as $root)
            <li>
                <a href="{{route('admin.product.category', ['category'=> $root->id])}}">{{$root->name}}</a>
            </li>
        @endforeach
    </ul>
    ...
    <table class="table table-bordered">
        ...
        @foreach($products as $product)
            <tr>
                <td>
                    <a href="{{route('admin.product.show', ['product'=> $product->id])}}">{{$product->name}}</a>
                </td>
